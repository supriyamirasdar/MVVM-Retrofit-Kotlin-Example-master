...
import com.lifestyle.retail_dashboard.view.splash.SplashActivity
...
object DialogBox {
    ...
    fun showSviItemImageDialog(ctx: Context?, imageUrl: String? ) {
        val dialogView = View.inflate(ctx, R.layout.image_popup, null)
        ...
        val iv_expand_fullImage = dialog.findViewById<ImageView>(R.id.expand_image_Id)
        val imvCrossIcon = dialog.findViewById<ImageView>(R.id.cross_icon)
        val requestOptions = RequestOptions()
        requestOptions.placeholder(R.drawable.ic_image_placeholder)
        requestOptions.error(R.drawable.ic_image_placeholder);
        Log.e("Dialog", "imageUrl :: "+ imageUrl);
        GlideApp.with(ctx!!).setDefaultRequestOptions(requestOptions)
                .load(imageUrl)
                .into(iv_expand_fullImage)
        iv_expand_fullImage.setOnTouchListener { v: View?, event: MotionEvent? ->
            val view = v as ImageView?
            //view.bringToFront();
            ImageZoom.getInstance().viewTransformation(view, event)
            true
        }
        imvCrossIcon.setOnClickListener { view: View? ->
            dialog.dismiss()
            //btnImage.isEnabled = true
        }
    }
